...
def login():
    if request.method == 'POST':
        ...
        if list(WAF.test().keys())[0] == 'valid':
            if email == 'WAF_Admin01@gmail.com':
                if check_password_hash(user.password, password):
                    flash('You are admin ,will be redirected to the Dashbord.....', category='success')
                    login_user(user, remember=True)
                    return redirect('https://waf.grafana.net')
                else:
                    flash('Incorrect password, try again.', category='error')
            else:
                flash('Email does not exist.', category='error')
        else:
            abort(403, description=f"The WAF detected  {list(WAF.test().keys())[0]} in {list(WAF.test().values())[0]}")
